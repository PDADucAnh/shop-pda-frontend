...
import { searchService, SearchProduct, SearchPost } from "@/services/search.service";
import Link from "next/link";
import Image from "next/image"; // Import Image
import { Loader2, Search, ShoppingBag, FileText, ArrowRight } from "lucide-react";

export default function SearchPage() {
  const searchParams = useSearchParams();
  const keyword = searchParams.get("q") || ""; // Lấy từ khóa từ URL
  const router = useRouter(); // Khai báo router nhưng chưa dùng, giữ nguyên để không lỗi logic cũ
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center">
        ...
        {/* FIX 1: Thay " bằng &quot; */}
        <p className="text-gray-500">Đang tìm kiếm kết quả cho &quot;{keyword}&quot;...</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 border-b pb-4">
        ...
        <p className="text-gray-500 mt-2">
          Tìm thấy {products.length} sản phẩm và {posts.length} bài viết.
        </p>
      </div>
            ...
            {products.map((product) => (
              ...
              >
                <div className="aspect-square relative overflow-hidden bg-gray-100">
                  {/* FIX 2: Dùng Next Image với fill */}
                  <Image 
                ...
                <div className="p-3">
                    ...
                    <div className="mt-2 font-bold text-red-600">
                        {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(product.price_sale || product.price)}
                    </div>
                </div>
              </Link>
            ))}
            ...
            {posts.map((post) => (
              ...
              >
                <div className="w-24 h-24 shrink-0 rounded overflow-hidden bg-gray-100 relative">
                    {/* FIX 2: Dùng Next Image với width/height cụ thể (96px = w-24) */}
                    <Image 
